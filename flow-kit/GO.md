...
> v1.10 新增：OMC 双模式自动切换

根据任务规模自动选择执行模式：

| 规模等级 | 触发条件               | 执行模式  | 说明                     |
| -------- | ---------------------- | --------- | ------------------------ |
| L0       | 单文件修改，<50行      | Autopilot | 极简模式，跳过 Phase 2-3 |
| L1       | 多文件修改，<200行     | Autopilot | 标准单人模式             |
| L2       | 跨模块改动，200-1000行 | Team      | 并行子代理模式           |
| L3       | 多模块协同，>1000行    | Team      | 全量流水线               |

### 自动切换协议

```
1. 读取 .flow-kit/scale-level 或自动估算改动范围
2. 对应选择 Autopilot / Team 模式
3. L0 跳过 Phase 2-3，直接进入极简流程
4. Team 模式自动调用 /flow-kit:dispatch 进行任务分发
```
...
```
/flow-kit:dispatch 3:"实现用户认证模块"  → Team 模式，3 个并行 executor
