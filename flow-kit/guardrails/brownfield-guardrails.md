...
**文件**：`@flow-kit/guardrails/performance-guardrails.md`

性能护栏检测，O(n²) 嵌套循环、N+1 查询、未使用索引的 SQL、阻塞主线程同步 IO。

### B6: Testing Coverage Gate

**文件**：`@flow-kit/guardrails/testing-coverage-gate.md`

测试覆盖率质量门禁，增量覆盖率 >=80% 方可通过。
